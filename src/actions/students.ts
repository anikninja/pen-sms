...
export async function createStudentAction(input: unknown) {
  return runAction(async () => {
    ...
    const student = await createStudent(parseInput(studentCreateSchema, input))
...
export async function updateStudentAction(studentId: unknown, input: unknown) {
  return runAction(async () => {
    await authorizeStaff()
    const student = await updateStudent(parseId(studentId, "Student"), parseInput(studentUpdateSchema, input))
    revalidatePath("/staff", "layout")
