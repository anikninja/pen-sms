...
> **Specification:** [docs/architecture.md](docs/architecture.md) is the full design (schema, rules, edge cases, decisions). Build status and verification records are in [docs/PROGRESS.md](docs/PROGRESS.md); a one-line history is in [docs/development_log.md](docs/development_log.md).
...
`createdAt` / `updatedAt` are omitted. Field rules are in [architecture.md §4–§16](docs/architecture.md#4-database-schema).
...
Every case gives a clear message or state — never a crash or raw database error. Full list: [architecture.md §25](docs/architecture.md#25-edge-cases).
...
The staff and student interfaces were also walked through in headless Chrome against a production build (sign-in, every workflow, access boundaries, no horizontal scroll at 375 / 768 / 1024 / 1280 px). Results for each phase are recorded in [docs/PROGRESS.md](docs/PROGRESS.md).
...
| CI and documentation | GitHub Actions workflow, progress records, this README |
